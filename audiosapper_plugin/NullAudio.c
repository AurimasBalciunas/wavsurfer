...
//	}
    ...
    else if (inOperationID == kAudioServerPlugInIOOperationWriteMix)
    {
        ...
        // If we have stored input, use it for the output
        if (gSharedBuffer != NULL)
        {
            FailWithAction(inIOBufferFrameSize*8 != gSharedBufferSize, theAnswer = kAudioHardwareBadObjectError, Done, "NullAudio_DoIOOperation: WriteMix size did not match shared buffer size.");
